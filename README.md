Python & SQL Learning Journey 🐍

This repository contains my initial projects as I build consistency and improve my coding skills step by step.

⭐Projects⭐
1. 7 Days Consistency:This project focuses on building a daily coding habit.
It is based on the idea of staying consistent and improving regularly.

2. Online Store SQL:This project is a simple database of an online store.
It includes:
Customers,
Products,
Orders.
Concepts used:
JOIN,
GROUP BY,
Aggregate functions.

3. Prime Number Checker:A simple Python program to check whether a number is prime or not.
Concepts used:
Loops (for),
Condition (if-else),
Modulus (%),
break & for-else.

4. Task master:A simple Python script to manage daily tasks with local file storage.

5. Quadratic Equation Solver:A lightweight Python utility to calculate the roots of a quadratic equation using the cmath library. This solver handles real, repeated, and complex roots seamlessly.

